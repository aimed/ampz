...
- (void)resetCursorRects
{
	...
	unsigned myLength = (unsigned)[URLStorage length];
...
- (NSURL*)urlAtMouse:(NSEvent *)mouseEvent
{
	...
	if ( ([URLStorage length] > 0 ) && [self mouse:mousePoint inRect:cellBounds] ) {
		...
		unsigned glyphIndex = (unsigned) [URLManager glyphIndexForPoint:mousePoint inTextContainer:URLContainer];
		unsigned charIndex = (unsigned)[URLManager characterIndexForGlyphAtIndex:glyphIndex];
		NSUInteger numRects = 0, j = 0;
...
- (void)mouseUp:(NSEvent *)mouseEvent
{
	...
	if ( (urlAtMouse != nil)  &&  [urlAtMouse isEqualTo:clickedURL] ) {
		...
		if (
            ...
            || ![textFieldDelegate respondsToSelector:@selector(textField:openURL:)]
            || ![textFieldDelegate textField:self openURL:urlAtMouse]
            )
...
-(void)setDelegate:(id<NSTextFieldDelegate,DSClickableURLTextFieldDelegate>)delegate {

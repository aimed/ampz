            ...
            compile: {
                options: {
                    data: function (dest, src) {
                        ...
                        var s = 'src/data/shared.json'
                        var n = src[0].replace('src/jade/','src/data/').replace('.jade','.json')

                        return {
                            debug: false,
                            shared: grunt.file.exists(s) ? grunt.file.readJSON(s) : {},
                            data: grunt.file.exists(n) ? grunt.file.readJSON(n) : {}
                        }
                    }
                },
                files: [
